import React, { useState, useEffect, useCallback } from 'react';import { convertirFechaHoraLocal, obtenerFechaLocal, obtenerFechaHoraLocalISO } from '../utils/dateFormatter';

const SUPABASE_URL = 'https://ubfkhtkmlvutwdivmoff.supabase.co';const SUPABASE_ANON_KEY = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InViZmtodGttbHZ1dHdkaXZtb2ZmIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NTA4MTc5NTUsImV4cCI6MjA2NjM5Mzk1NX0.c0iRma-dnlL29OR3ffq34nmZuj_ViApBTMG-6PEX_B4';

const AhorrosModule = ({ idSocio: propIdSocio }) => {const [ahorrosList, setAhorrosList] = useState([]);const [sociosList, setSociosList] = useState([]);const [loading, setLoading] = useState(true);const [error, setError] = useState(null);const [showAddAhorroModal, setShowAddAhorroModal] = useState(false);const [showDetailsModal, setShowDetailsModal] = useState(false);const [showEditModal, setShowEditModal] = useState(false);const [selectedSocioAhorros, setSelectedSocioAhorros] = useState(null);const [editingAhorro, setEditingAhorro] = useState(null);const [newAhorro, setNewAhorro] = useState({id_socio: '',ahorro_aportado: '',multa_hoja_monto: '',afiliacion_papeleria_monto: '',fecha: obtenerFechaLocal()});const [toastMessage, setToastMessage] = useState('');const [totalSociosConAhorro, setTotalSociosConAhorro] = useState(0);const [totalAhorroAcumulado, setTotalAhorroAcumulado] = useState(0);const [searchTerm, setSearchTerm] = useState('');const [filteredAhorrosBySocio, setFilteredAhorrosBySocio] = useState([]);const [showRetiroModal, setShowRetiroModal] = useState(false); // se mantiene (sin botón para abrir)const [retiroMonto, setRetiroMonto] = useState('');const [socioParaRetiro, setSocioParaRetiro] = useState(null);

const currentUserRole = localStorage.getItem('currentUser') ? JSON.parse(localStorage.getItem('currentUser')).role : '';

useEffect(() => {fetchGlobalAhorroStats();fetchSocios();
...
useEffect(() => {const lowerCaseSearchTerm = searchTerm.toLowerCase();const filtered = sociosList.filter(socio => {const fullName = ${socio.nombre} ${socio.apellido_paterno} ${socio.apellido_materno}.toLowerCase();return (fullName.includes(lowerCaseSearchTerm) ||socio.id_socio.toString().includes(lowerCaseSearchTerm));}).map(socio => {const socioAhorros = ahorrosList.filter(ahorro => ahorro.id_socio === socio.id_socio);const ahorroAcumuladoSocio = socioAhorros.reduce((sum, item) => sum + (parseFloat(item.ahorro_aportado) || 0), 0);return {id_socio: socio.id_socio,nombre_completo: ${socio.nombre} ${socio.apellido_paterno} ${socio.apellido_materno},ahorro_acumulado: ahorroAcumuladoSocio,ahorros_detalles: socioAhorros};});setFilteredAhorrosBySocio(filtered);}, [searchTerm, sociosList, ahorrosList]);

const fetchGlobalAhorroStats = async () => {try {const sociosConAhorroResponse = await fetch(${SUPABASE_URL}/rest/v1/ahorros?select=id_socio, {method: 'GET',headers: {'Content-Type': 'application/json',apikey: SUPABASE_ANON_KEY,Authorization: Bearer ${SUPABASE_ANON_KEY}}});if (!sociosConAhorroResponse.ok) {const errorData = await sociosConAhorroResponse.json();throw new Error(Error al cargar socios con ahorro: ${sociosConAhorroResponse.statusText} - ${errorData.message || 'Error desconocido'});}const sociosConAhorroData = await sociosConAhorroResponse.json();const uniqueSociosIds = new Set(sociosConAhorroData.map(item => item.id_socio));setTotalSociosConAhorro(uniqueSociosIds.size);
...
const fetchAhorrosForUser = async (socioId) => {setLoading(true);setError(null);try {const url = ${SUPABASE_URL}/rest/v1/ahorros?id_socio=eq.${socioId}&select=*;const response = await fetch(url, {method: 'GET',headers: {'Content-Type': 'application/json',apikey: SUPABASE_ANON_KEY,Authorization: Bearer ${SUPABASE_ANON_KEY}}});if (!response.ok) {const errorData = await response.json();throw new Error(Error al cargar ahorros: ${response.statusText} - ${errorData.message || 'Error desconocido'});}const data = await response.json();setAhorrosList(data);} catch (err) {setError(err.message);} finally {setLoading(false);}};

const fetchAhorrosForAllSocios = async () => {setLoading(true);setError(null);try {const url = ${SUPABASE_URL}/rest/v1/ahorros?select=*;const response = await fetch(url, {method: 'GET',headers: {'Content-Type': 'application/json',apikey: SUPABASE_ANON_KEY,Authorization: Bearer ${SUPABASE_ANON_KEY}}});if (!response.ok) {const errorData = await response.json();throw new Error(Error al cargar ahorros: ${response.statusText} - ${errorData.message || 'Error desconocido'});}const data = await response.json();setAhorrosList(data);} catch (err) {setError(err.message);} finally {setLoading(false);}};

const fetchSocios = async () => {try {const response = await fetch(${SUPABASE_URL}/rest/v1/socios?select=id_socio,nombre,apellido_paterno,apellido_materno, {method: 'GET',headers: {'Content-Type': 'application/json',apikey: SUPABASE_ANON_KEY,Authorization: Bearer ${SUPABASE_ANON_KEY}}});if (!response.ok) {const errorData = await response.json();throw new Error(Error al cargar socios: ${response.statusText} - ${errorData.message || 'Error desconocido'});}const data = await response.json();setSociosList(data);} catch (err) {console.error("Error al cargar socios para el modal:", err);}};

const handleAddInputChange = (e) => {const { name, value } = e.target;setNewAhorro(prev => ({ ...prev, [name]: value }));};

const handleRegisterAhorro = async (e) => {e.preventDefault();setLoading(true);setError(null);setToastMessage('');
...
const ahora = new Date();const horaLocal =String(ahora.getHours()).padStart(2, '0') +':' +String(ahora.getMinutes()).padStart(2, '0') +':' +String(ahora.getSeconds()).padStart(2, '0');
...
id_socio: '',ahorro_aportado: '',multa_hoja_monto: '',afiliacion_papeleria_monto: '',fecha: obtenerFechaLocal()});fetchGlobalAhorroStats();if (currentUserRole === 'usuario' && propIdSocio) {fetchAhorrosForUser(propIdSocio);} else if (currentUserRole === 'admin') {fetchAhorrosForAllSocios();}
...
const handleEditInputChange = (e) => {const { name, value } = e.target;setEditingAhorro(prev => ({ ...prev, [name]: value }));};

const handleUpdateAhorro = async (e) => {e.preventDefault();setLoading(true);setError(null);setToastMessage('');
...
const formatCurrency = (value) => {return new Intl.NumberFormat('es-MX', { style: 'currency', currency: 'MXN' }).format(value);};

const handleVerDetalles = (socioAhorro) => {setSelectedSocioAhorros(socioAhorro);setShowDetailsModal(true);};

const handleEditarAhorro = (ahorro) => {setEditingAhorro({id_ahorro: ahorro.id_ahorro,ahorro_aportado: ahorro.ahorro_aportado,fecha: ahorro.fecha});setShowEditModal(true);};

// --- Flujos de retiro se mantienen (sin botón que los dispare) ---const handleRetirarAhorroClick = (socioAhorro) => {setSocioParaRetiro(socioAhorro);setRetiroMonto('');setShowRetiroModal(true);};const handleRetiroMontoChange = (e) => setRetiroMonto(e.target.value);

const handleAplicarRetiro = async () => {const monto = parseFloat(retiroMonto);if (isNaN(monto) || monto <= 0) {alert('Por favor, ingrese un monto válido para retirar.');return;}if (monto > socioParaRetiro.ahorro_acumulado) {alert('El monto a retirar no puede ser mayor al ahorro acumulado.');return;}if (window.confirm(¿Estás seguro que deseas retirar ${formatCurrency(monto)} del ahorro de ${socioParaRetiro.nombre_completo}?)) {setLoading(true);setError(null);setToastMessage('');try {const currentDate = new Date();const fechaRetiro = obtenerFechaHoraLocalISO();
...
fecha_hora: ${obtenerFechaLocal()}T${new Date().toTimeString().slice(0, 8)}-06:00,es_retiro: true};
