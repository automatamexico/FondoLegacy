...
const SociosModule = () => {
  ...
  const [editingSocio, setEditingSocio] = useState(null); // Para almacenar el socio que se está editando
  ...
  const handleAddOrUpdateSocio = async (e) => {
    ...
    try {
      ...
      if (editingSocio) {
        ...
        if (!response.ok) {
          const errorData = await response.json();
          throw new Error(`Error al registrar socio: ${socioResponse.statusText} - ${errorData.message || 'Error desconocido'}`);
        }
        addedOrUpdatedSocio = await response.json();
