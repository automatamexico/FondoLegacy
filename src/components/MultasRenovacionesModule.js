...
const [showAfiliacionesPagadas, setShowAfiliacionesPagadas] = useState(false);
  const [busquedaAfiliacion, setBusquedaAfiliacion] = useState('');
...
const afiliacionesFiltradas = useMemo(() => {
  ...
  if (!texto) return afiliacionesPagadas;

  return afiliacionesPagadas.filter((pago) => {
    const id = String(pago.id_socio || '').toLowerCase();
    const nombre = String(pago.nombreCompleto || '').toLowerCase();

    return id.includes(texto) || nombre.includes(texto);
  });
}, [afiliacionesPagadas, busquedaAfiliacion]);
  return (
...
{showAfiliacionesPagadas && (
  ...
  <p className="text-sm text-slate-500">
    Total de registros: {afiliacionesFiltradas.length}
  </p>

  <input
    type="text"
    value={busquedaAfiliacion}
    onChange={(e) => setBusquedaAfiliacion(e.target.value)}
    placeholder="Buscar por ID o nombre..."
    className="w-full mt-3 px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-emerald-500"
  />
</div>
        <button
          type="button"
          onClick={() => setShowAfiliacionesPagadas(false)}
          className="shrink-0 px-3 py-2 rounded-lg bg-slate-100 text-slate-700"
        >
          Cerrar
        </button>
      </div>
        ...
        {afiliacionesPagadas.length === 0 ? (
          ...
          <>
            ...
            <div className="md:hidden space-y-3">
              {afiliacionesFiltradas.map((pago, index) => (
                <div
